...
// Feladat02 
match (p:Person) 
where p.born = 1964 or p.born = 1965 // vagy where p.born in  [1964,1965]
...
WITH p.name As Nev, count(m.title) As Darab  //segéd lekérdezés mindig új nevet kell neki adni ,group by nincs
Where Darab > 1
Return Nev, Darab
```
...
```js
//Feladat06: listázzuk akik egyszerre szereplők és rendezők egy filben 
